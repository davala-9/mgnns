...
if __name__ == "__main__":
        ...
        if alpha > 0:
            ...
            for j in relevant_positions_xy_0_for_xy_1[i]:
                if gnn_output_gd[0][row_ab][j].item() == 1:
                    ...
                    defining_rule_body_neg.add(("X", iclr_encoder_decoder.get_data_predicate(
                        can_encoder_decoder.position_unary_pred_dict[j]), "Y"))
            for j in relevant_positions_yx_0_for_xy_1[i]:
                if gnn_output_gd[0][row_ba][j].item() == 1:
                    ...
                    defining_rule_body_neg.add(("Y", iclr_encoder_decoder.get_data_predicate(
                        can_encoder_decoder.position_unary_pred_dict[j]), "X"))
            rules.add((defining_rule_head, frozenset(defining_rule_body_pos), frozenset(defining_rule_body_neg)),)
        ...
        if alpha > 0:
            ...
            for j in relevant_positions_zx_0_for_x_1[i]:
                ...
                if contributing_neighbour_exists:
                    ...
                    defining_rule_body_neg.add((z, iclr_encoder_decoder.get_data_predicate(
                        can_encoder_decoder.position_unary_pred_dict[j]), "X"))
            rules.add((defining_rule_head, frozenset(defining_rule_body_pos), frozenset(defining_rule_body_neg)),)
            ...
            for j in relevant_positions_xy_0_for_yx_1[i]:
                if gnn_output_gd[0][row_ab][j].item() == 1:
                    ...
                    defining_rule_body_neg.add(("Y", iclr_encoder_decoder.get_data_predicate(
                        can_encoder_decoder.position_unary_pred_dict[j]), "X"))
            rules.add((defining_rule_head, frozenset(defining_rule_body_pos), frozenset(defining_rule_body_neg)),)
    rules.add((("X", iclr_encoder_decoder.get_data_predicate(cd_fact_predicate), "Y"), frozenset(layer_1_to_2_rule_body), frozenset([])),)
    ...
    for (head_atom, pos_body_atoms, neg_body_atoms) in rules:
        body_atoms = []
        ...
        for (s, p, o) in neg_body_atoms:
            if p == type_pred:
                body_atoms.append("NOT <{}>[?{}]".format(o, s))
            else:
                body_atoms.append("NOT <{}>[?{},?{}]".format(p, s, o))
