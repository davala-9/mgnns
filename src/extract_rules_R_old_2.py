...
if __name__ == "__main__":
    ...
    # one relevant unary that is not present).
        ...
        max_neg = []
        min_pos = []
        frontier = [(1, (0,))]  # pair: first element is number 1 (variable X1) and second is a tuple that just contains 0
        counter = 0
        depth_first = False
        while frontier:
            if (counter % 1) == 0:
                print("{} nodes explored".format(counter))
            counter += 1
            #print("frontier")
            #print(frontier)
            # body_candidate = frontier.pop(random.randrange(len(frontier)))
            if depth_first:
                body_candidate = frontier.pop(-1)
            else:
                body_candidate = frontier.pop(0)
            if evaluate_rule(i, body_candidate):
                depth_first = False
                #print("captured")
                min_pos = [conj for conj in min_pos if is_smaller_or_equal(body_candidate, conj)]
                min_pos.append(body_candidate)
            else:
                #print("not_captured")
                depth_first = True
                max_neg = [conj for conj in max_neg if is_smaller_or_equal(body_candidate, conj)]
                max_neg.append(body_candidate)
                filtered_frontier = []
                potential_successors = get_successors(body_candidate)
                for frontier_elem in frontier:
                    if is_smaller_or_equal(frontier_elem, body_candidate):
                        for succ in get_successors(frontier_elem):
                            if is_smaller_or_equal(succ, body_candidate):
                                potential_successors += (succ,)
                    else:
                        #print(frontier_elem)
                        filtered_frontier += (frontier_elem,)
                frontier = filtered_frontier
                for conj in min_pos:  # Iterate once over min_pos since it is probably the largest set.
                    potential_successors = [pot_succ for pot_succ in potential_successors if not is_smaller_or_equal(conj,pot_succ)]
                #print("successors")
                    # print(potential_successors)
                frontier += potential_successors
