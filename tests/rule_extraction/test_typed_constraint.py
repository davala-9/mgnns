...
class TestGroupedConstraints:
    def test_fresh_instances_do_not_share_mutable_defaults(self):
        a = GroupedConstraints()
        b = GroupedConstraints()
        a.always_one.add("D")
        assert b.always_one == set()
...
class TestEdgeConstraints:
    ...
    def test_setters_only_affect_their_own_type_pair(self):
        tc = TypedConstraint({ROOT, NONROOT})
        tc.set_edge_always_exists(ROOT, NONROOT, "part_of")
