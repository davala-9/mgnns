...
def test_root_has_no_unary_predicate():
    ...
    tree = matrix_to_tree(SparseTriangularMatrix.empty(d=3, max_value=2), AdniSignature(encoder))
...
def test_rejects_a_matrix_whose_dimensions_do_not_match_the_signature():
    signature = AdniSignature(make_internal_encoder(d=3, max_value=2))
    with pytest.raises(ValueError):
        matrix_to_tree(SparseTriangularMatrix.empty(d=2, max_value=2), signature)
    with pytest.raises(ValueError):
        matrix_to_tree(SparseTriangularMatrix.empty(d=3, max_value=1), signature)
...
def test_included_nodes_excludes_the_others_entirely():
    matrix = SparseTriangularMatrix.empty(d=3, max_value=2).with_value(0, 2, 1)
    encoder = make_internal_encoder(d=3, max_value=2)
    tree = matrix_to_tree(matrix, AdniSignature(encoder), included_nodes={0, 2})
...
def test_included_nodes_drops_matrix_entries_touching_an_excluded_node():
    ...
    encoder = make_internal_encoder(d=3, max_value=2)
    tree = matrix_to_tree(matrix, AdniSignature(encoder), included_nodes={0, 2})  # excludes node_1, the entry's target
    node_0_id = 1
